		...
		case 'SaveOrUpdate':
			...
			if(move_uploaded_file($imagen, "../Files/Global/".$ruta)){
				if(empty($_POST["txtIdGlobal"])){
					...
					if($objGlobal->Registrar($empresa,$nombre_impuesto,$porcentaje_impuesto,$simbolo_moneda,$margen, "Files/Global/".$ruta)){
					...
					if($objGlobal->Modificar($idglobal, $empresa,$nombre_impuesto,$porcentaje_impuesto,$simbolo_moneda,$margen, "Files/Global/".$ruta)){
				...
				if(empty($_POST["txtIdGlobal"])){
					...
					if($objGlobal->Registrar($empresa,$nombre_impuesto,$porcentaje_impuesto,$simbolo_moneda,$margen, $ruta_img)){
						echo "Configuración Global registrada, se utilizará esta configuración.";
					}else{
						echo "La configuración global no ha podido ser registada.";
					}
					...
					$idglobal = $_POST["txtIdGlobal"];
					if($objGlobal->Modificar($idglobal, $empresa,$nombre_impuesto,$porcentaje_impuesto,$simbolo_moneda,$margen, $ruta_img)){
						echo "La configuración Global ha sido actualizada";
					}else{
						echo "la Informacion de la configuración global no ha podido ser actualizada.";
					}
            ...
            $i = 1;
     		while ($reg = $query_Tipo->fetch_object()) {
     			$data[] = array($i,
					...
					'<button class="btn btn-warning" data-toggle="tooltip" title="Editar" onclick="cargarDataGlobal('.$reg->idglobal.',\''.$reg->empresa.'\',\''.$reg->nombre_impuesto.'\',\''.$reg->porcentaje_impuesto.'\',\''.$reg->simbolo_moneda.'\',\''.$reg->margen.'\',\''.$reg->logo.'\')"><i class="fa fa-pencil"></i> </button>&nbsp;'
					);
				$i++;
			}
