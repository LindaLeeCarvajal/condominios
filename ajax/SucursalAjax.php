	...
	switch ($_GET["op"]) {
		...
		case 'SaveOrUpdate':
			...
			$estado = $_POST["txtEstado"];
				$numero_autorizacion = $_POST["txtNumeroAutorizacion"];
					$leyenda_facturacion = $_POST["txtLeyenda"];
						$llave_dosificacion = $_POST["txtLlaveDosificacion"];
							$fecha_limite_emision_facturas = $_POST["fechaLimiteEmision"];
			...
			if(move_uploaded_file($imagen, "../Files/Sucursal/".$ruta)){
				...
				,$numero_autorizacion,$leyenda_facturacion,$llave_dosificacion,$fecha_limite_emision_facturas)){
					...
					if($objSucursal->Modificar($idsucursal, $razon_social,$tipo_documento,$num_documento,$direccion,$telefono,$email,$representante,"Files/Sucursal/".$ruta,$estado,$numero_autorizacion,$leyenda_facturacion,$llave_dosificacion,$fecha_limite_emision_facturas)){
				...
				if(empty($_POST["txtIdSucursal"])){
					...
					if($objSucursal->Registrar($razon_social,$tipo_documento,$num_documento,$direccion,$telefono,$email,$representante, $ruta_img,$estado	,$numero_autorizacion,$leyenda_facturacion,$llave_dosificacion,$fecha_limite_emision_facturas)){
						echo "Sucursal Registrada correctamente.";
					}else{
						echo "Sucursal no ha podido ser registada.";
					}
					...
					$idsucursal = $_POST["txtIdSucursal"];
					if($objSucursal->Modificar($idsucursal,$razon_social,$tipo_documento,$num_documento,$direccion,$telefono,$email,$representante, $ruta_img,$estado,$numero_autorizacion,$leyenda_facturacion,$llave_dosificacion,$fecha_limite_emision_facturas)){
						echo "La información de la Sucursal ha sido actualizada.";
					}else{
						echo "La información de la Sucursal no ha podido ser actualizada.";
					}
            ...
            $i = 1;
     		while ($reg = $query_Tipo->fetch_object()) {
     			...
     			$data[] = array($i,
					...
					'<button class="btn btn-warning" data-toggle="tooltip" title="Editar" onclick="cargarDataSucursal('.$reg->idsucursal.',\''.$reg->razon_social.'\',\''.$reg->tipo_documento.'\',\''.$reg->num_documento.'\',\''.$reg->direccion.'\',\''.$reg->telefono.'\',\''.$reg->email.'\',\''.$reg->representante.'\',\''.$reg->logo.'\',\''.$reg->estado.'\',\''.$reg->numero_autorizacion.'\',\''.$reg->leyenda_facturas.'\',\''.$reg->llave_dosificacion.'\',\''.$reg->fecha_limite_emision.'\')"><i class="fa fa-pencil"></i> </button>&nbsp;'.
