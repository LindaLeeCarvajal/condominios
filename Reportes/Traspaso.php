...
class PDF_Traspaso extends FPDF
{
// private variables
var $colonnes;
var $format;
...
// Company
function addSociete( $nom, $adresse,$logo,$ext_logo,$y1 )
{
	$x1 = 30;
//	$y1 = 8;
	...
	if ($y1==8) {
		...
		$this->Cell( $length, 2, 'ORIGEN: ' . $nom);
	} else {
//		$this->Image($logo , 5 ,33, 25 , 25 , $ext_logo);
		$this->SetXY( $x1, $y1 );
		$this->SetFont('Arial','B',12);
		$length = $this->GetStringWidth( $nom );
		$this->Cell( $length, 2, 'DESTINO: ' . $nom);
	}
	$this->SetXY( $x1, $y1 + 4 );
...
function addDate( $date, $nombreusuario )
{
	$r1  = $this->w - 100;
	$r2  = $r1 + 90;
	$y1  = 17;
	...
	$this->Cell(10,5, "Fecha: " . $date, 0, 0, "C");
	$this->SetXY( $r1 + ($r2-$r1)/2 - 5, $y1+9 );
	$this->SetFont( "Arial", "B", 10);
	$this->Cell(10,5,'Realizado por: ' . $nombreusuario, 0,0, "C");
}
...
function addMotivo( $motivo )
{
	...
	$this->SetFont( "Arial", "", 10);
	$this->Cell(5,5,"Motivo: " . utf8_decode($motivo), 0,0, "L");

//	$length = $this->GetStringWidth( $motivo );
//	$lignes = $this->sizeOfText( $motivo, $length) ;
//	$this->MultiCell($length, 4, $motivo);
...
function addCadreTVAs($monto)
{
	...
	$this->Cell(10,4, "IMPORTE TOTAL EN LITERAL:");
...
function addCadreEurosFrancs($impuesto)
{
	...
	$this->SetXY( $r1, $y1+4 );
		$this->Cell(20,9, "DESCUENTO(%)", 0, 0, "L");
	$this->SetXY( $r1, $y1+5 );
	$this->Cell(20,14, "TOTAL A PAGAR", 0, 0, "L");
}
...
//                  "qte"     => qte,
//                  "tva"     => code_tva );
function addTVAs( $descuento, $total,$moneda )
{

	//******CALCULO DEL DESCUENTO************
	$totalc = ($total)-(($total*$descuento)/100);
	$this->SetFont('Arial','',8);
//*************FIN DEL CALCULO***************
	$re  = $this->w - 30;
	$rf  = $this->w - 29;
	...
	$this->SetXY( $re, $y1+4 );
	$this->Cell( 17,9 , $descuento."%", '', '', 'R');
	$this->SetXY( $re, $y1+5 );
	$this->Cell( 17,14, $moneda.sprintf("%0.2F", $totalc), '', '', 'R');
