...
function addCadreTVAs($monto,$pago,$moneda,$cliente)

{
	...
	$this->Cell(10,4, "HEMOS RECIBIDO DE:  ".$cliente);

	$this->SetFont( "Arial", "", 9);

	$this->SetXY( $r1+9, $y1+7);

	$this->MultiCell(100,7, $monto);

	$this->SetXY( $r1+10, $y1+8);

	$this->Cell( 13,17, $moneda.sprintf("%0.2F", $pago), '', '', 'R');
...

{
	...
	$this -> Cell(0,5,'COMPROBANTE DE PAGO',0,0,'C');

}

function UsuarioFirma($usuario)

{
	...
	$this-> Cell(1,7, $usuario,0,0,'C');

	$this->SetXY( $r1, $y1);

	$this -> SetFont( "Arial", "B", 10);

	$this -> Cell(0,5,'USUARIO',0,0,'C');
...

{
	...
	$this->Cell(20,4, "TOTAL VENTA", 0, 0, "L");

	$this->SetXY( $r1, $y1+4 );

		$this->Cell(20,9, "TOTAL PAGADO", 0, 0, "L");

	$this->SetXY( $r1, $y1+5 );

	$this->Cell(20,14, "SALDO TOTAL", 0, 0, "L");
...
function addTVAs( $descuento, $total,$moneda,$totalpagado)
...
//*************FIN DEL CALCULO***************
	...
	$this->Cell( 17,4, $moneda. $totalc, '', '', 'R');

	$this->SetXY( $re, $y1+4 );

	$this->Cell( 17,9, $moneda.sprintf("%0.2F", $totalpagado), '', '', 'R');

	$this->SetXY( $re, $y1+5 );

		$this->Cell( 17,14, $moneda.sprintf("%0.2F", $totalsaldo), '', '', 'R');
