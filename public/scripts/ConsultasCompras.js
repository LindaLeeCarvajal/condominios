...
function init(){
    ...
    var tablaKV = $("#tblKardexValorizado").dataTable({
		...
		var tablaKV = $("#tb1librocompra").dataTable({
        dom: 'Bfrtip',
        buttons: [
            'copyHtml5',
            'excelHtml5',
	...
	ListadoStockArticulos();
		...
		$("#date_lv_desde").click(libro_ventas);
		$("#date_lv_hasta").click(libro_ventas);
...
if($("#date_lv_desde").val() != "" && $("#date_lv_hasta").val() != ""){

		var fecha_desde = $("#date_lv_desde").val(), fecha_hasta = $("#date_lv_hasta").val(), idsucursal = $("#txtIdSucursal").val();

		var tabla = $('#tb1librocompra').dataTable(
		{   "aProcessing": true,
					"aServerSide": true,
					dom: 'Bfrtip',
					buttons: [
					...
					"aoColumns":[
						...
						{
							url: './ajax/ConsultasComprasAjax.php?op=libro_ventas',
					type : "get",
