...
function init() {
    ...
    $("#btnAgregarArtPed").click(function (e) {
        ...
        opt.each(function () {
            AgregarDetallePed($(this).val(), $(this).attr("data-nombre"), $(this).attr("data-precio-venta"), "1", "0", $(this).attr("data-stock-actual"), $(this).attr("data-descripcionPed"), $(this).attr("data-tipoPrecioPed"), $(this).attr("data-idarticulo"));
        })
...
//$("#VerListado").show();
    ...
    function AgregarDetallePed(iddet_ing, nombre, precio_venta, cant, descripcion, stock_actual, tipo_precio, idarticulo) {
        var detalles = new Array(iddet_ing, nombre, precio_venta, cant, descripcion, stock_actual, tipo_precio, idarticulo);
...
function AgregarPedCarrito(iddet_ing, stock_actual, art, descripcion, tipo_precio, precio_venta, stock_detalle, idarticulo) {
    console.log(idarticulo)
    if (stock_detalle > 0) {
        if (tipo_precio == "Precio Tienda") {
            var detalles = new Array(iddet_ing, art, precio_venta, "1", "Precio Tienda", stock_actual, "", "Precio Tienda", stock_detalle, idarticulo);
            elementos.push(detalles);
            ConsultarDetallesPed();
        } else {
            if (tipo_precio == "Precio X Mayor") {
                var detalles = new Array(iddet_ing, art, precio_venta, "1", "Precio X Mayor", stock_actual, "", "Precio X Mayor", stock_detalle, idarticulo);
                elementos.push(detalles);
                ConsultarDetallesPed();
            } else {
                if (tipo_precio == "Precio Auspicio") {
                    var detalles = new Array(iddet_ing, art, precio_venta, "1", "Precio Auspicio", stock_actual, "", "Precio Auspicio", stock_detalle, idarticulo);
                    elementos.push(detalles);
                    ConsultarDetallesPed();
                } else {
                    if (tipo_precio == "Precio Distribuidor") {
                        var detalles = new Array(iddet_ing, art, precio_venta, "1", "Precio Distribuidor", stock_actual, "", "Precio Distribuidor", stock_detalle, idarticulo);
                        elementos.push(detalles);
                        ConsultarDetallesPed();
                    }
                }
